...
# "import" statement.
dependencies = (Dependency(module="PySide2", package=None, name=None), Dependency(module="imageio", package=None, name=None), )
...
class GAOLIB_OT_uninstall_dependencies(bpy.types.Operator):
    ...
    def execute(self, context):
        try:
            install_pip()
            for dependency in dependencies:
                uninstall_module(module_name=dependency.module,
                                          package_name=dependency.package,
                                          global_name=dependency.name)
        except (subprocess.CalledProcessError, ImportError) as err:
            self.report({"ERROR"}, str(err))
...
preference_classes = (GAOLIB_PT_warning_panel,
...
class OT_CreatePose(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not bpy.context.scene.gaolib_tool.gaolibNewPose:
            ShowMessageBox("This tool can ONLY be used with GAOLIB after creating a new Pose. (Camera Button)", 'ABORT')
            return{'CANCELLED'}
...
class OT_CreateAnimation(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not bpy.context.scene.gaolib_tool.gaolibNewAnimation:
            print("This tool can ONLY be used with GAOLIB after creating a new Animation. (Camera Button)")
            ShowMessageBox("This tool can ONLY be used with GAOLIB after creating a new Animation. (Camera Button)", 'ABORT')
            return{'CANCELLED'}
        ...
        jsonFile = os.path.join(os.path.dirname(os.path.dirname(bpy.context.scene.render.filepath)), 'temp.json')
        with open(jsonFile, 'w') as file:
            json.dump(data, file, indent=4, sort_keys=True)
        ...
        for bone in bpy.context.selected_pose_bones:
            ...
            for axis in range(3):
                if not bone.lock_location[axis]:
                    bone.keyframe_insert(data_path='location', index=axis, frame=frameIn)
                if not bone.lock_rotation[axis]:
                    bone.keyframe_insert(data_path='rotation_euler', index=axis, frame=frameIn)  
        ...
        if bpy.context.scene.gaolib_tool.gaolibKeyLastFrame:
            ...
            for bone in bpy.context.selected_pose_bones:
                ...
                for axis in range(3):
                    if not bone.lock_location[axis]:
                        bone.keyframe_insert(data_path='location', index=axis, frame=frameOut)
                    if not bone.lock_rotation[axis]:
                        bone.keyframe_insert(data_path='rotation_euler', index=axis, frame=frameOut)  
                    if not bone.lock_scale[axis]:
                        bone.keyframe_insert(data_path='scale', index=axis, frame=frameOut)
                for key in bone.keys():
                    try:
                        bone.keyframe_insert(data_path='["' + key + '"]', frame=frameOut)
                    except Exception as e:
                        pass
...
class GaolibCustomProperties(bpy.types.PropertyGroup):
    ...
    gaolibKeyLastFrame: bpy.props.BoolProperty(
        name='keyLastFrame',
        default=False)

    gaolibNewAnimation: bpy.props.BoolProperty(
        name='newAnimation',
        default=False)

    gaolibNewPose: bpy.props.BoolProperty(
        name='newPose',
        default=False)
...
class VIEW3D_PT_Gaolib(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col4.operator('development.create_pose',
                      text='Create Pose',
                      icon='POSE_HLT')
        col4.operator('development.create_animation',
                      text='Create Animation',
                      icon='ANIM')
        col4.operator('development.show_overlay_params',
